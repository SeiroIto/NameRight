...
vcov_CRSeiro <- function(obj, cluster, type, target = NULL, inverse_var = FALSE, form = "sandwich", ignore_FE = FALSE) {
  cluster <- droplevels(as.factor(cluster))
  # In S3-methods.R (88), coef_CS <- function(obj) UseMethod("coef_CS") 
  # In S3-methods.R (92), coef_CS.default <- function(obj) {coef(obj)}
  # So below gives NA slots in coef(obj) for lm objects
  alias <- is.na(coef_CS(obj))
  # Similarly, S3-methods.R (40) fetches model.matrix(obj) as default.
  X <- model_matrix(obj)
  if (any(alias)) {
    X <- X[, !alias, drop = FALSE]
  }  
  ...
  cluster_length <- length(cluster)
  if (cluster_length != N) {
    # handle_vectors drops obs with NAs or zero weight
    cluster <- droplevels(handle_vectors(cluster, obj))
    if (length(cluster) != N) {
      stop("Clustering variable must have length equal to the number of rows in the data used to fit obj.")
    }
  ...
  J <- nlevels(cluster)
  if (J < 2) stop("Cluster-robust variance estimation will not work when the data only includes a single cluster.")
  # matrix-function.R (13): 
  #  Split rows by cluster, return a block diagonal matrix of X (block[i]=cluster[i])
  X_list <- matrix_list(X, cluster, "row")
  W_list <- weightMatrix(obj, cluster)
  # Apply weigtung to all elements of X_list and W_list
  XW_list <- Map(function(x, w) as.matrix(t(x) %*% w), x = X_list, w = W_list)
  ...
  if (is.null(target)) {
   ...
   # S3-methods.R (5): targetVariance (when inverse_var == F) is 
   # a K list of diag(rep(1, n_{cluster_k})), cluster k = 1,...,K.
